...
## Overview
This application demonstrates the use of an earphone with MIC to record audio.

## Instructions
To demonstrate the app, follow these steps:

1. Build and download the application to the AIROC&#8482; board.
2. Press and hold the CUSTOM button to record audio via an earphone with MIC plugged into J10, with J19 shortened.(Maximum recording time 30 secs)
3. Press and hold the VOL- button to play the recorded audio.

For CYW920721M2EVK-02, PDM, on-board DMIC, can be used as audio input by setting ENABLE\_PDM=1 flag.
When enabled, the base board SW4 must be switched to DMIC. The C-compiler flag AUDIO\_RECORD\_ON\_PDM\_INPUT will
be automatically enabled in makefile.
