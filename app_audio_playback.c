...
#define AUDIO_INSERT_NB_SAMPLES        128
#define AUDIO_INSERT_BUFFER_SIZE       (AUDIO_INSERT_NB_SAMPLES * AUDIO_INSERT_NB_CHANNELS)
...
typedef struct
{
    ...
    int16_t buffer[AUDIO_INSERT_BUFFER_SIZE]; /* 128 Frames, 256 Samples, 512 bytes */
...
void decode_thread_main_loop(void)
{
    ...
    do
    {
        status = app_opus_decode(pcm_buffer, &compress_buffer[compress_buffer_current], AUDIO_INSERT_NB_SAMPLES, &consumed_size);
        if (status != WICED_SUCCESS)
        {
...
static void app_audio_playback_pcm_data_add_noncompressed(void)
{
    ...
    if (nb_frames > AUDIO_INSERT_NB_SAMPLES)
    {
        nb_frames = AUDIO_INSERT_NB_SAMPLES;
    }
    else
    {
